...
#include  "hz16x16.h"  //16*16汉字库
//#include  "hz64x64.h"  //64*64汉字库
...
void LCD_ShowChar(u16 x,u32 y,u8 num,u8 size,u8 mode)
{  							  
    u8 temp,t1,t;
	u16 y0=y;
	...
	{   
		...
		{			    
			...
			else if(mode==0)LCD_Fast_DrawPoint(x,y,BLACK);   //字符背景色，可以自己随意改
			temp<<=1;
			y++;
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowString(u16 x,u32 y,u8 size,u8 *p)
{         
	//u8 x0=x;
	//width+=x;
	//height+=y;
    ...
    {       
        ...
        LCD_ShowChar(x,y,*p,size,0);  //0为非叠加方式
        x+=size/2;
        p++;
    }  
}
...
void showhfstr(unsigned int x1,unsigned int y1,unsigned char *str,unsigned int dcolor,unsigned int bgcolor)	  
{  
	 unsigned char l=0;
	while(*str)
	{	
...
////写入单个汉字
//void PutHZ6464(unsigned short x, unsigned short  y, unsigned char c[2], unsigned int dcolor,unsigned int bgcolor)
//{
//	unsigned int i,j,k;
//																//c[2] 把要显示的字 以两个字节的形式放入 数组c中
//	LCD_Set_Window(x,y,64,64);
//	LCD_SetCursor(x,y);
//	LCD_WriteRAM_Prepare();

//	for (k=0;k<512;k++) 
//	{ //64标示自建汉字库中的个数，循环查询内码		  一个汉字需要两个字节
//	  if ((codeHZ_64[k].Index[0]==c[0])&&(codeHZ_64[k].Index[1]==c[1]))	    //寻找对应汉字
//	  
//	  { 
//    	for(i=0;i<512;i++) 
//			{
//				unsigned int m=codeHZ_64[k].Msk[i];
//				for(j=0;j<8;j++) 
//				{
//					if((m&0x80)==0x80) 
//					{
...
//void showhz64str(unsigned int x1,unsigned int y1,unsigned char *str,unsigned int dcolor,unsigned int bgcolor)	 
//     {  unsigned char l=0;
//        while(*str)
//           {
//     	   PutHZ6464(x1+l*32,y1,(unsigned char*)str,dcolor, bgcolor);
//			str+=2;l+=2;
//      }
//    }
